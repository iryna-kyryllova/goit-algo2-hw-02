...
def rod_cutting_memo(length: int, prices: List[int]) -> Dict:
    ...
    memo = {}  # Обчислені значення
    cuts_memo = {}  # Шляхи розрізання

    def get_max_profit(rod_length):
        if rod_length == 0:
            return 0, []
        if rod_length in memo:
            return memo[rod_length], cuts_memo[rod_length]
    ...
    max_profit, cuts = get_max_profit(length)
    return {
